...
import { signPhotoNames } from "./photosig.mjs";
...
function compact(labelled, photoSecret) {
  return {
    i: labelled.placeId,
    n: labelled.name,
    a: labelled.address,
    ...
    // Signed here rather than in the client, so a discovered photo is fetchable
    // for exactly the same reason a saved one is: we minted the name.
    ph: labelled.photos?.length ? signPhotoNames(labelled.photos, photoSecret) : undefined,
    onList: false,
  };
}
...
export async function handleDiscover(body, env = process.env) {
  ...
  const photoSecret = (env.PHOTO_SECRET ?? "").trim();
  ...
  return {
    ...
    body: {
      ...
      places: places
        ...
        .map((r) => compact(labelPlacesResult(r), photoSecret)),
