 ...
 *
 * The route stays unauthenticated — an <img> tag cannot carry a passcode, and a
 * per-device token would fragment the CDN cache that makes photos free. What
 * stops it being an open tap on our billing is the signature: a name is only
 * forwarded to Google if we signed it. See photosig.mjs.
 */

import { verifyPhotoSignature } from "./photosig.mjs";
...
export async function resolvePhoto(query, env = process.env) {
  ...
  // The shape check above says the name is well-formed, not that it is ours.
  // Without this, any harvested Google photo name would bill our key, so the
  // signature is checked before anything reaches the network. Unset secret
  // means local dev, matching how APP_PASSCODE leaves the API routes open.
  const secret = (env.PHOTO_SECRET ?? "").trim();
  if (secret && !verifyPhotoSignature(name, query?.s, secret)) {
    return { status: 403, error: "unsigned photo name" };
  }
